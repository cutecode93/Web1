...
// function checkWinner(sc1, sc2, sc3, sc4, sc5, sc6) {
...
//   } else {
//     console.log('NO TEAM WINS LOL');
//   }
// }

// console.log(checkWinner(85, 54, 41, 23, 34, 27));

// 어쨌든 함수가 작동하긴 하는데 너무 구구절절 장황한느낌스~~?
...
//     console.log(`Koala wins {${avgDolphins} vs ${avgKoalas}}`);
//   } else {
...
2. Create a 'calcBMI' method(=객체 내 함수) on each object to calculate the BMI (the same method on both objects). Store the BMI value to a property, and also return it from the method.
...
function total(tips, bills) {
  return tips + bills
}

for (let i = 0; i < 10; i++) { // i is not defined 라는 오류가 계속뜬다...미치게따
  console.log(calctip(bills[i]) + bills[i]); // 해결!!!
}
